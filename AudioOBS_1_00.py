...
def _program_dir() -> Path:
    """
    Directory of the actual program on disk — this is where AudioOBS.json
    is read and written.

    It MUST be the folder the user sees the .exe in, never a temporary
    folder. Nuitka --onefile extracts the program into a temp folder that
    is wiped when the program closes; writing AudioOBS.json there would
    silently lose every setting on exit.

    - Nuitka build (standalone AND onefile): the module object
      ``__compiled__`` provides ``.containing_dir`` — the directory of the
      REAL .exe.  IMPORTANT: Nuitka does NOT set ``sys.frozen``, so that
      flag must never be used to detect a Nuitka build.
    - PyInstaller-style frozen build: ``sys.executable`` is the real .exe.
    - Plain ``python AudioOBS_1_00.py`` run: the directory of this file.
    """
    try:
        return Path(__compiled__.containing_dir).resolve()   # Nuitka build
    except NameError:
        pass
    if getattr(sys, "frozen", False):                        # PyInstaller
        return Path(sys.executable).resolve().parent
    return Path(__file__).resolve().parent                   # plain script


def _bundle_dir() -> Path:
    """
    Directory of the bundled data files (resources/icons/...).

    Unlike :func:`_program_dir`, bundled resources live where the *code*
    runs from. For a Nuitka --onefile build that is the temp extraction
    folder (``--include-data-dir`` copies them there); for a standalone
    build and a plain script run it is the same folder as this file.
    ``__file__`` points at exactly the right place in all three cases.
    """
    return Path(__file__).resolve().parent


SETTINGS_FILE = _program_dir() / "AudioOBS.json"
