...
def _is_frozen() -> bool:
    """
    True when running as a packaged build rather than a plain
    `python AudioOBS_1_00.py` invocation.

    Covers both packagers:
      * Nuitka  - sets the module-level global __compiled__
                  (it does NOT set sys.frozen).
...
def _persist_dir() -> Path:
    """
    Directory for files that must SURVIVE between launches - i.e.
    AudioOBS.json. This is the folder that holds the real .exe on disk
    (frozen) or the script (source run).

    Crucially this is NOT the extraction folder. A Nuitka --onefile
    build unpacks itself into a fresh %TEMP%\\onefile_* directory on
    every launch and wipes it on exit; writing AudioOBS.json there
    would silently lose every setting between runs (the symptom: the
    log says "AudioOBS.json not found, creating with defaults" on
    every single start). sys.executable is the actual .exe on disk,
    so that is what we anchor to.
    """
    if _is_frozen():
...
def _bundle_dir() -> Path:
    """
    Directory for READ-ONLY files shipped *inside* the build - the
    toucan icons and logos bundled via Nuitka's --include-data-dir.

    This is the opposite of _persist_dir() for a --onefile build: the
    bundled data is extracted next to __file__ (the temporary
    onefile_* folder), NOT next to the .exe. Using __file__ here makes
    the icons resolve correctly whether the app is frozen or run from
    source, since in both cases the resources sit beside the module.
    """
    return Path(__file__).resolve().parent


# AudioOBS.json lives next to the .exe so it persists across launches.
SETTINGS_FILE = _persist_dir() / "AudioOBS.json"
# How many distinct "Wait for:" exe names to keep in the editable
# wait-for combo box's dropdown. Newest entries push out the oldest.
# 10 is plenty: more than that and the dropdown stops being a quick
# pick and starts being a list to scroll.
_WAIT_HISTORY_MAX = 10
...
LOGO_PATH        = _bundle_dir() / "resources" / "icons" / "logo.png"
...
STYLED_LOGO_PATH = _bundle_dir() / "resources" / "icons" / "logo_styled.png"
...
ICON_PATH        = _bundle_dir() / "resources" / "icons" / "AudioOBS.ico"
...
class C:
    ...
    LEVEL_HIGH     = "#e74c3c"
    LEVEL_EMPTY    = "#3a4870"
    # Peak-hold marker. A bright cyan reads clearly as a separate
    # indicator against the green/yellow bar and the dark empty cells,
    # without the harshness of pure white. Drawn as a FULL braille
    # cell so it shows up as a solid vertical tick, the way a hardware
    # PPM peak indicator looks.
    LEVEL_PEAK     = "#22d3ee"
...
class VuMeter(QWidget):
    BRAILLE_FILLED = "\u28ff"
    BRAILLE_EMPTY  = "\u28c0"
    # Peak-hold marker glyph. U+28FF - the FULL braille cell, same as
    # BRAILLE_FILLED. Drawn in C.LEVEL_PEAK (bright cyan) it reads as a
    # solid, unmistakable vertical tick. The old U+28B6 ("\u28b6", a
    # sparse 4-dot pattern) looked like a stray fragment of bar rather
    # than a deliberate marker, especially once peak-hold leaves a gap
    # between the bar and the held peak.
    BRAILLE_PEAK   = "\u28ff"
    ...
    # ===== Scale calibration =========================================
    # AudioOBS captures an already-mastered digital stream, so the
    # signal can never exceed 0 dBFS. The meter therefore runs on a
    # 0 dBFS-anchored scale; "+4 dB" headroom (a hold-over from an
    # analogue-style peak meter) made the top of the bar physically
    # unreachable and is gone.
    #
    # The BAR shows RMS (average loudness). RMS of real music sits
    # ~12-20 dB below the sample peaks.
    #
    # The dB floor is NOT fixed - it scales continuously with the bar
    # length (i.e. the window width). The reasoning: a short bar has
    # few cells, so a deep floor would put many dB into each cell and
    # squash the useful range; a long bar has the resolution to show
    # much more depth, right down to the noise floor, for free.
    #   - narrowest bar  (BAR_LEN_MIN cells) -> DB_FLOOR_NARROW (-40)
    #   - widest bar     (BAR_LEN_MAX cells) -> DB_FLOOR_WIDE   (-90)
    #   - in between     -> linearly interpolated
    # _refresh_scale() does the interpolation; _db_to_meter() reads the
    # resulting per-instance _db_floor / _db_range.
    DB_FLOOR_NARROW = -40.0        # floor at the narrowest window
    DB_FLOOR_WIDE   = -90.0        # floor at the widest window
    DB_CEILING      =   0.0
    # Bar-length clamp range - must match the max(20, min(80, ...)) in
    # resizeEvent(). The floor interpolation maps this span onto the
    # DB_FLOOR_NARROW..DB_FLOOR_WIDE range.
    BAR_LEN_MIN     = 20
    BAR_LEN_MAX     = 80
    # Back-compat alias: external callers / tests referencing DB_FLOOR
    # still get a sane value (the narrow-window default).
    DB_FLOOR        = DB_FLOOR_NARROW

    # ===== Peak-marker over-level thresholds =========================
    # The peak MARKER is driven by the true sample peak, not the RMS,
    # and its colour warns about over-level on a real dBFS scale:
    #   cyan   : peak <  -3 dBFS   - normal headroom
    #   orange : -3 dBFS .. 0 dBFS - hot, close to clipping
    #   red    : peak >=  0 dBFS   - at/over full scale, clipping risk
    # These compare against the peak's dB value directly (not the
    # mapped bar position), so they stay correct regardless of the
    # bar's -40..0 window.
    PEAK_ORANGE_DBFS = -3.0
    PEAK_RED_DBFS    =  0.0

    # ---- Peak-hold timing -------------------------------------------
    # set_levels() is driven by the 40 ms VU timer, i.e. 25 ticks/sec.
    # PEAK_HOLD_TICKS  : how long the marker sits still after a new peak.
    #                    ~1.5 s of dwell = 38 ticks - the classic value
    #                    you see on hardware PPMs and DAW meters.
    # PEAK_FALL_PER_TICK: how far (in 0..1 meter units) the marker drops
    #                    each tick once the hold has expired. 0.012/tick
    #                    at 25 Hz empties the full scale in ~3.3 s, a
    #                    calm, readable fall.
    PEAK_TICK_MS       = 40
    PEAK_HOLD_TICKS    = 38           # ~1.5 s at 40 ms/tick
    PEAK_FALL_PER_TICK = 0.012

    def __init__(self, parent=None):
        super().__init__(parent)
        self._meter_l = 0.0
        self._meter_r = 0.0
        # ---- Classic studio peak-hold -------------------------------
        # The peak marker SNAPS up to any new high instantly, then sits
        # perfectly still for PEAK_HOLD_TICKS ticks, and only after that
        # holding period does it start to fall (PEAK_FALL_PER_TICK per
        # tick). This is the behaviour of hardware PPM meters and the
        # peak indicators in DAWs - the eye gets a still target to read
        # instead of a value that slides away the instant it appears.
        #
        # We hold the peak as a meter-space value (0..1, same scale as
        # the bar) plus a per-channel countdown of remaining hold ticks.
        self._peak_l = 0.0
        self._peak_r = 0.0
        self._peak_hold_l = 0          # ticks left to hold current peak
        self._peak_hold_r = 0
        # Current peak-marker colour per channel - cyan / orange / red,
        # chosen from the live sample-peak dBFS in set_levels().
        self._peak_col_l = C.LEVEL_PEAK
        self._peak_col_r = C.LEVEL_PEAK
        ...
        self._bar_len = 45
        # ---- Dynamic scale ------------------------------------------
        # The dB floor adapts to the bar length: a wide window gets the
        # -90 dB floor (more cells = enough resolution to show it), a
        # narrow one stays at -60. _refresh_scale() recomputes these
        # whenever _bar_len changes; _db_to_meter() reads them.
        self._db_floor = self.DB_FLOOR
        self._db_range = self.DB_CEILING - self.DB_FLOOR
        # Bar-length fraction at which the yellow (loud) zone begins -
        # the -10 dBFS RMS mark. Recomputed in _refresh_scale().
        self._zone_green_end = 0.75
    ...
    def _refresh_scale(self) -> None:
        """
        Recompute the dB floor and zone split for the current bar length.

        The floor moves CONTINUOUSLY with the bar length: at BAR_LEN_MIN
        cells it sits at DB_FLOOR_NARROW (-40 dB), at BAR_LEN_MAX it
        reaches DB_FLOOR_WIDE (-90 dB), and it interpolates linearly in
        between. So as the user drags the window wider the meter gains
        depth smoothly - no visible jump. The yellow zone always begins
        at the -10 dBFS RMS mark, wherever that lands on the active
        scale.
        """
        span = self.BAR_LEN_MAX - self.BAR_LEN_MIN
        if span > 0:
            t = (self._bar_len - self.BAR_LEN_MIN) / span
        else:
            t = 0.0
        t = max(0.0, min(1.0, t))                    # clamp 0..1
        self._db_floor = (self.DB_FLOOR_NARROW
                          + t * (self.DB_FLOOR_WIDE - self.DB_FLOOR_NARROW))
        self._db_range = self.DB_CEILING - self._db_floor
        # Yellow (loud) zone starts at -10 dBFS - its 0..1 position on
        # the current scale.
        self._zone_green_end = max(0.0, min(1.0,
            (-10.0 - self._db_floor) / self._db_range))

    def _db_to_meter(self, db: float) -> float:
        """
        Map a dB value linearly onto the bar's 0..1 range using the
        CURRENT (bar-length-dependent) _db_floor.._db_range window.
        """
        return max(0.0, min(1.0,
                            (db - self._db_floor) / self._db_range))

    @staticmethod
    def _amp_to_db(amp: float) -> float:
        """Linear amplitude (0..1+) to dBFS. Returns -inf-ish floor for ~0."""
        if amp < 1e-7:
            return -999.0
        return 20.0 * math.log10(amp)

    def _rms_to_meter(self, rms: float) -> float:
        # Thin wrapper kept so external callers / tests still work.
        return self._db_to_meter(self._amp_to_db(rms))

    @staticmethod
    def _db_text(amp: float) -> str:
        """Format a linear amplitude as a dBFS label, or -inf when silent."""
        if amp < 1e-7:
            return "-\u221e dB"
        return f"{20.0 * math.log10(amp):+5.1f} dB"

    def _peak_marker_color(self, peak_amp: float) -> str:
        """
        Pick the peak-marker colour from the true sample-peak level:
          cyan   - normal headroom (peak below -3 dBFS)
          orange - hot, within 3 dB of full scale
          red    - at or over 0 dBFS, clipping risk
        """
        db = self._amp_to_db(peak_amp)
        if db >= self.PEAK_RED_DBFS:
            return C.LEVEL_HIGH        # red
        if db >= self.PEAK_ORANGE_DBFS:
            return C.LEVEL_MID         # orange/amber
        return C.LEVEL_PEAK            # cyan

    def set_levels(self, rms_l: float, rms_r: float,
                   peak_l: float = 0.0, peak_r: float = 0.0) -> None:
        """
        Update the meter.

        rms_l/r  - average level, drives the BAR (slow, "loudness").
        peak_l/r - true sample peak, drives the PEAK MARKER (hold-then-
                   fall) and its over-level colour. Defaults to 0 so a
                   caller that only has RMS still works (marker then
                   simply tracks the RMS, as before).
        """
        # ---- Bar: RMS, instant rise + smooth slow-fall --------------
        rms_meter_l = self._db_to_meter(self._amp_to_db(rms_l))
        rms_meter_r = self._db_to_meter(self._amp_to_db(rms_r))
        self._meter_l = max(rms_meter_l, self._meter_l * self._meter_fall)
        self._meter_r = max(rms_meter_r, self._meter_r * self._meter_fall)

        # ---- Peak marker: true sample peak, classic hold-then-fall --
        # If the caller passed no peak, fall back to the RMS so the
        # marker still has something sensible to track.
        pk_l = peak_l if peak_l > 0.0 else rms_l
        pk_r = peak_r if peak_r > 0.0 else rms_r
        pk_meter_l = self._db_to_meter(self._amp_to_db(pk_l))
        pk_meter_r = self._db_to_meter(self._amp_to_db(pk_r))
        self._peak_l, self._peak_hold_l = self._update_peak(
            pk_meter_l, self._peak_l, self._peak_hold_l)
        self._peak_r, self._peak_hold_r = self._update_peak(
            pk_meter_r, self._peak_r, self._peak_hold_r)
        # Marker colour follows the LIVE sample peak, not the held bar
        # position - so a brief red flash on a transient is visible
        # even though the marker itself is still gliding down.
        self._peak_col_l = self._peak_marker_color(pk_l)
        self._peak_col_r = self._peak_marker_color(pk_r)

        self._render(self._meter_l, self._meter_r)
        # dB readout shows the PEAK value (the meaningful number when
        # judging headroom / clipping), not the RMS.
        self.db_l.setText(self._db_text(pk_l))
        self.db_r.setText(self._db_text(pk_r))

    def _update_peak(self, level: float, peak: float, hold: int):
        """
        Advance one channel's peak marker by a single tick.

        Returns the (new_peak, new_hold_counter) pair.

        Rules - the classic PPM behaviour:
          * A level at or above the current peak SNAPS the marker up to
            it and re-arms the full hold timer. The marker is now frozen.
          * While the hold counter is positive it just counts down; the
            marker does not move at all.
          * Once the hold counter reaches zero the marker falls by a
            fixed step each tick until it meets the live level again.
        """
        if level >= peak:
            # New high (or equal) - jump up and freeze.
            return level, self.PEAK_HOLD_TICKS
        if hold > 0:
            # Still within the dwell window - hold perfectly still.
            return peak, hold - 1
        # Hold expired - fall at a constant rate, but never below the
        # current level (so the marker rejoins the bar instead of
        # diving past it).
        return max(level, peak - self.PEAK_FALL_PER_TICK), 0

    def reset(self) -> None:
        self._meter_l = self._meter_r = 0.0
        self._peak_l  = self._peak_r  = 0.0
        self._peak_hold_l = self._peak_hold_r = 0
        self._peak_col_l = self._peak_col_r = C.LEVEL_PEAK
    ...
    def resizeEvent(self, event) -> None:
        ...
        new_len = max(self.BAR_LEN_MIN,
                      min(self.BAR_LEN_MAX, available // char_w - 1))
        if new_len != self._bar_len:
            self._bar_len = new_len
            # Bar length drives the dB floor - recompute the scale so a
            # wider window gets the deeper floor (down to -90 dB).
            self._refresh_scale()
    ...
    def _bar_html(self, level: float, peak: float, peak_color: str) -> str:
        n = self._bar_len
        ...
        # The BAR (RMS) now has just two zones - green up to -10 dB RMS,
        # yellow above. Over-level warning lives entirely on the peak
        # MARKER (orange/red), which is the indicator that can actually
        # tell clipping apart from a merely loud average.
        green_max = int(self._zone_green_end * n)

        parts: List[str] = []

        # ---- filled section: green then yellow ----
        gc = max(0, min(filled, green_max))
        if gc:
            parts.append(f'<span style="color:{C.LEVEL_LOW};">'
                         f'{self.BRAILLE_FILLED * gc}</span>')
        yc = max(0, filled - green_max)
        if yc:
        ...
        # peak_color is chosen by the caller from the true sample-peak
        # dBFS value: cyan (normal) / orange (-3..0) / red (>= 0).
        if show_peak:
            ...
            parts.append(f'<span style="color:{peak_color};">'
    ...
    def _render(self, level_l: float, level_r: float) -> None:
        ...
        new_l = self._bar_html(level_l, self._peak_l, self._peak_col_l)
        if new_l != self._last_html_l:
            self.bar_l.setText(new_l)
            self._last_html_l = new_l
        new_r = self._bar_html(level_r, self._peak_r, self._peak_col_r)
...
class AudioOBSMainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # True sample-peak per channel, updated by the audio callbacks
        # alongside the RMS. Drives the VU meter's peak-hold marker and
        # its over-level (orange/red) colouring. Guarded by _rms_lock.
        self._cur_peak_l: float = 0.0
        self._cur_peak_r: float = 0.0
        ...
        # Most-recently-used list of "Wait for:" executable names. Backs
        # the editable wait-for combo box's dropdown so the user can
        # re-pick an app they've waited for before instead of retyping
        # the exe name. Persisted in AudioOBS.json (key "wait_for_history")
        # and kept self-maintaining - newest first, de-duplicated
        # case-insensitively, capped at _WAIT_HISTORY_MAX entries.
        self._wait_for_history: list[str] = []
    ...
    def _apply_remembered_source(self) -> None:
        ...
        self.wait_name.setCurrentText(exe)
    ...
    def _build_ui(self) -> None:
        ...
        # 640 px is the comfortable default - at 540 the button strip
        # ("Start Recording" / scissors / "Sequential") gets visibly
        # cramped, 600 is the floor at which everything breathes, and
        # 640 leaves a touch of extra room. Minimum is set just below
        # the default so the layout can't be squeezed past what the
        # button row needs.
        self.setMinimumSize(560, 820)
        self.resize(640, 880)
        # Cap the width at 3x the default so the VU meter and the other
        # widgets don't get stretched into an awkwardly long strip.
        # 3 * 640 = 1920 - exactly one Full-HD screen width. Height
        # stays unconstrained.
    ...
    def _build_source_card(self) -> QGroupBox:
        ...
        self.process_combo.setToolTip(
            ...
            "here \u2013 an app that is open but silent will not appear until it\n"
            "plays a sound. The list refreshes automatically every 2 seconds.\n"
            "To capture an app before it makes any sound, use \u201cWait for:\u201d."
        )
        ...
        # Inline hint shown ONLY when the process list is empty.
        # WASAPI enumerates audio *sessions*, not processes: an app
        # that is open but silent has no session and therefore cannot
        # appear here. Rather than leave the user staring at an empty
        # combo box, explain why and point at the two ways forward -
        # start playback, or arm "Wait for:" before the app makes a
        # sound. _refresh_processes() toggles its visibility.
        self.source_hint = QLabel(
            "No app is playing audio yet. An application only appears here "
            "once it actually starts playing sound \u2013 start playback in the "
            "app, or tick \u201cWait for:\u201d below and type its .exe name to arm "
            "capture before it plays."
        )
        self.source_hint.setObjectName("sourceHint")
        self.source_hint.setWordWrap(True)
        self.source_hint.setVisible(False)
        v.addWidget(self.source_hint)
        ...
        # Page 1 (checkbox CHECKED) - the editable wait-for combo box.
        #         The user types (or picks from the dropdown) the exe
        #         name to wait for and the wait_timer keeps polling
        #         enumerate_sessions for it. The dropdown is populated
        #         from _wait_for_history so previously-used apps can be
        #         re-picked without retyping.
        ...
        # Editable combo box: behaves like a text field for typing, but
        # the drop-down arrow exposes the most-recently-used exe names.
        self.wait_name = QComboBox()
        self.wait_name.setEditable(True)
        self.wait_name.setMaximumWidth(260)
        self.wait_name.setInsertPolicy(QComboBox.InsertPolicy.NoInsert)
        # An empty dropdown looks broken, so the line-edit placeholder
        # tells first-time users what to type.
        self.wait_name.lineEdit().setPlaceholderText("e.g. spotify.exe")
        self.wait_name.setToolTip(
            ...
            "The drop-down lists apps you have waited for before."
        )
        self.source_info_stack.addWidget(self.wait_name)          # index 1

        row2.addWidget(self.source_info_stack, 1)

        # Checkbox toggles which page is shown. The wait_name combo box
        # stays alive behind the scenes when it's hidden, so whatever
        # the user typed or picked is preserved if they re-tick the box.
    ...
    def _apply_settings(self, data: dict) -> None:
        ...
        if "window_size" in data:
            try:
                ...
                # Clamp to the same bounds setMinimumSize/setMaximumWidth
                # enforce in _build_ui, so a stale AudioOBS.json from an
                # earlier layout (e.g. the old 820-wide default, or a
                # window dragged wider than the new 1920 cap) can never
                # restore an out-of-range geometry.
                w = max(560, min(1920, int(w)))
                h = max(820, int(h))
                self.resize(w, h)
            except Exception:
                pass
        ...
        if "last_source_exe" in data:
            try:
                ...
                pass
        # ----- wait_for_history (dropdown of past wait targets) -----
        # Loaded BEFORE wait_for_exe so the combo box is already
        # populated when we set its current text below. Each entry is
        # sanity-checked: must be a short, non-empty string. A corrupt
        # file simply yields an empty / shorter history, never a crash.
        if "wait_for_history" in data:
            try:
                raw = data["wait_for_history"]
                if isinstance(raw, list):
                    cleaned: list[str] = []
                    for item in raw:
                        s = str(item).strip()
                        if s and len(s) <= 260 and s.lower() not in (
                            c.lower() for c in cleaned
                        ):
                            cleaned.append(s)
                        if len(cleaned) >= _WAIT_HISTORY_MAX:
                            break
                    self._wait_for_history = cleaned
                    self._populate_wait_combo()
            except Exception:
                pass
        ...
        if "wait_for_exe" in data:
            try:
                ...
                # settings file can't put 5 MB of text in the combo box.
                if 0 < len(exe) <= 260:
                    self.wait_name.setCurrentText(exe)
            except Exception:
                pass
    ...
    def _save_settings(self) -> None:
        ...
        data = {
            ...
            "wait_for_exe":      self.wait_name.currentText().strip() if hasattr(self, "wait_name") else "",
            # MRU dropdown for the "Wait for:" combo box. Self-maintaining
            # list, newest first - see _add_wait_history().
            "wait_for_history":  list(self._wait_for_history),
    ...
    def _wire_settings_persistence(self) -> None:
        ...
        # Track every other piece of UI state too, so the JSON acts as
        # a full save-state. The wait-for combo box uses a 500 ms
        # debouncer so typing doesn't beat up the disk; editTextChanged
        # fires both on keystrokes and on dropdown picks.
        self.wait_cb.toggled.connect(self._save_settings)
        self.wait_name.editTextChanged.connect(
    ...
    def _refresh_processes(self) -> None:
        ...
        if not sessions and not self.recording_started:
            self.process_combo.addItem(
                "  \u2014  no app is playing audio  \u2014", userData=(0, ""))

        # Show the explanatory hint only while the list is genuinely
        # empty. Once at least one audio session exists (or a recording
        # is running) the combo box is self-explanatory, so we hide it
        # again to keep the Source card compact.
        if hasattr(self, "source_hint"):
            self.source_hint.setVisible(
                not sessions and not self.recording_started
            )
    ...
    def _stop_capture_silent(self) -> None:
        ...
        with self._rms_lock:
            ...
            self._cur_peak_l = self._cur_peak_r = 0.0
        if hasattr(self, "vu"):
            self.vu.reset()
    ...
    # ===== Wait-for-app =============================================
    def _populate_wait_combo(self) -> None:
        """
        Refill the wait-for combo box's dropdown from _wait_for_history
        without disturbing whatever the user has currently typed.

        The combo box is editable, so its "current text" is the live
        edit field - we must preserve it across the clear()/addItems()
        or the user's half-typed exe name would vanish. Signals are
        blocked so this repopulation does not trigger a settings save.
        """
        if not hasattr(self, "wait_name"):
            return
        current = self.wait_name.currentText()
        self.wait_name.blockSignals(True)
        self.wait_name.clear()
        self.wait_name.addItems(self._wait_for_history)
        # Restore the edit text exactly as it was.
        self.wait_name.setCurrentText(current)
        self.wait_name.blockSignals(False)

    def _add_wait_history(self, exe: str) -> None:
        """
        Insert an executable name at the front of the wait-for MRU list,
        de-duplicated case-insensitively, capped at _WAIT_HISTORY_MAX.
        Repopulates the dropdown and persists the change.

        Called when a "Wait for:" target is actually found - that's the
        signal the entry is worth remembering. Typing alone does not add
        to the history (avoids polluting it with typos).
        """
        exe = (exe or "").strip()
        if not exe or len(exe) > 260:
            return
        # Drop any existing entry with the same name (case-insensitive)
        # so the freshly-used one moves to the top instead of duplicating.
        low = exe.lower()
        self._wait_for_history = [
            e for e in self._wait_for_history if e.lower() != low
        ]
        self._wait_for_history.insert(0, exe)
        del self._wait_for_history[_WAIT_HISTORY_MAX:]
        self._populate_wait_combo()
        self._save_settings()
    ...
    def _check_wait_for_app(self) -> None:
        ...
        if not wca:
            return
        name = self.wait_name.currentText().strip().lower()
        if not name:
            return
        try:
            sessions = wca.enumerate_sessions()
        ...
        for s in sessions:
            if s.executable.lower() == name:
                ...
                # The wait actually paid off - remember this exe in the
                # dropdown's history so it can be re-picked next time.
                # We store the real executable name as reported by the
                # session (correct casing) rather than the user's typed
                # text.
                self._add_wait_history(s.executable)
            ...
            try:
                if channels > 1:
                    ...
                    if n > 0:
                        ...
                        # True sample peak per channel - the largest
                        # absolute sample value in this block. This is
                        # what actually clips at 0 dBFS, so it (not the
                        # RMS) drives the peak-hold marker and the
                        # orange/red over-level colouring.
                        peak_l = float(np.max(np.abs(l)))
                        peak_r = float(np.max(np.abs(r)))
                    else:
                        rms_l = rms_r = peak_l = peak_r = 0.0
                else:
                    n = arr.size
                    if n > 0:
                        rms_l = rms_r = math.sqrt(float(np.dot(arr, arr)) / n)
                        peak_l = peak_r = float(np.max(np.abs(arr)))
                    else:
                        rms_l = rms_r = peak_l = peak_r = 0.0
                with self._rms_lock:
                    self._cur_rms_l = rms_l
                    self._cur_rms_r = rms_r
                    self._cur_peak_l = peak_l
                    self._cur_peak_r = peak_r
            except Exception:
                pass
    ...
    def _update_vu(self) -> None:
        with self._rms_lock:
            ...
            pl, pr = self._cur_peak_l, self._cur_peak_r
        self.vu.set_levels(l, r, pl, pr)
