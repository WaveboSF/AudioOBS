...
class AudioOBSMainWindow(QMainWindow):
    ...
    def _open_ffmpeg(self, out_path: Path) -> bool:
        ...
        try:
            ...
            self.log("ERROR", "ffmpeg not found on PATH. Install ffmpeg and try again.")
            QMessageBox.critical(self, "ffmpeg missing",
                                 "ffmpeg.exe was not found on your PATH.")
    ...
    def _toggle_meter_view(self) -> None:
        ...
        history from the last time the panel was visible.
        """
        new_idx = (self.meter_stack.currentIndex() + 1) % 2
        if new_idx == 1:
            # Going to spectrogram - reset to blank and mark as visible
            # so audio callbacks start feeding the FFT queue.
            self.spec.clear()
